...
def load_sentiment_model():
    try:
        # Advanced tricky sentiment detection
...
if sentiment_model:
    try:
        test = sentiment_model("I feel great today!", candidate_labels=["positive", "negative", "neutral"])
        st.sidebar.success(f"✅ AI model ready: {test['labels'][0]} ({test['scores'][0]:.2f})")
    except Exception as e:
        ...
        if st.button("Save & Analyze"):
            if journal.strip():
                if len(journal.split()) < 10:
                    ...
                    try:
                        result = sentiment_model(journal.strip(), candidate_labels=["positive", "negative", "neutral"])
                        ...
                        st.markdown(f"**Sentiment:** {sentiment_label.upper()}")
                    except Exception as e:
